...
processed_pdf_text = None

# Initialize the LLM model and do the embedding stuff
def initialize_llm():
   global llm, llm_embeddings
   """Initializes the language model and embeddings.
...
# Processing the PDF document
def process_pdf(pdf_path, llm):
    global llm_embeddings, conversation_retrieval_chain, processed_pdf_text
    """Processes a PDF file and creates a conversational retrieval chain.
...
process_pdf(pdf_path, llm)

# Now, you can use the process_prompt function to interact with the conversational retrieval chain.
while True:
        prompt = input("Ask a question about the document: ")
        chat_history = []
        if prompt == "quit":
